...
export default class NeuralNetwork extends GPUConnector {
  ...
  createBuffers() {
    ...
    this.createBuffer(
      'Layer_Derivatives',
      Float32Array.BYTES_PER_ELEMENT * this.maxLayer * (2 * this.maxLayer + 3) * this.layers.length,
      GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST | GPUBufferUsage.COPY_SRC
    );
    this.createBuffer(
      `Network_Step`,
  ...
  fillLayerBuffer(layerIndex, neuronData) {
    if (!neuronData) {
      neuronData = [
        ...
        Array.from({length: this.layers[layerIndex]}, () => (Math.random() * 2 - 1)),
        // Array(this.layers[layerIndex]).fill(layerIndex ? 1: 0),
        Array.from({length: (this.layers[layerIndex - 1] || 0) * this.layers[layerIndex]}, () => (Math.random() * 2 - 1)),
  ...
  async extractNetworkParameters() {
    ...
    const tempInpBi = await this.mapBufferToCPU(`Neuron_Inputs&Biases_copy`, Float32Array);
    params.inputs = tempInpBi.slice(0, tempInpBi.length / 2);
    params.biases = tempInpBi.slice(tempInpBi.length / 2);


    const allWeights = await this.mapBufferToCPU(`Neuron_Weights_copy`, Float32Array);
  ...
  async extractNetworkOutput() {
    ...
    this.copyBuffer(`Outputs`, commandEncoder, offset, offset);
    this.device.queue.submit([commandEncoder.finish()]);

    const outputResult = await this.mapBufferToCPU(`Outputs_copy`, Float32Array);
  ...
  async #predictionLoop(points, pointIndex, predictions, res) {
    ...
    while(pointIndex < points.length && (performance.now() - start) <= MAX_TRAINING_INTERRUPT) {
      ...
      this.forward();
      predictions.push(await this.extractNetworkOutput());
    }
