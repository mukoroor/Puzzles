...
export const move_eval_shader = (cubeDim, moves) => {
    ...
    return /*wgsl*/ `
        ...
        const moves: array<${CUBE_MOVE_STRUCT_NAME}, ${movesCount}>  = array(${movesString});

        struct ${CUBE_MOVE_STRUCT_NAME} {
            face: u32,
            depth: u32,
            rotCount: u32,
        }
        ...
        fn scoreDP(cubeInd: u32, faceInd: u32) -> f32 {
            ...
            for (var i: u32 = 0; i < dimension; i++) {
                for (var j: u32 = 0; j < dimension; j++) {
                    ...
                    if ((i != 0 && diffRowVal == val) && (j != 0 && diffColVal == val) && idCache[j] != idCurr[j - 1] && idCache[j - 1] != idCache[j]) {
                        idCurr[j] = idCurr[j - 1];
                        scoring[val]--;
                        count--;
