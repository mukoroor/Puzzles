...
export default class RubixPuzzleDrawer extends Drawer {
    ...
    #generatePieceVisitSequenceAndPositions() {
        ...
        const removalQueue = [length === 1 ? this.puzzle.faces[5].center :this.puzzle.faces[5].corners[3]];
        const sequence = [];
        const positions = [];
        ...
        while(removalQueue.length) {
            ...
            for (const [faceIndex, face] of currPiece.faceData.entries()) {
                ...
                const position = positions[pieceMap.get(currPiece.id)].map((e, i) => e + delta[i])
