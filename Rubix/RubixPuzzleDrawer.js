...
export default class RubixPuzzleDrawer extends Drawer {
  ...
  constructor() {
    ...
    this.setUpKeyListener(
      (type) => type === '?',
      () => {
        this.colors = generateDistinctColors(6);
...
function generateDistinctColors(count) {
    ...
    for (let i = 0; i < count; i++) {
        const hue = (i + Math.random() * 0.5) / count;  // Spread hues evenly across the spectrum
        const saturation = 0.7 + Math.random() * 0.3; // Moderate saturation to avoid overly bright or dull colors
        const lightness = 0.6 + Math.random() * 0.2;  // Higher lightness to avoid dark colors
        
        const rgb = hslToRgb(hue * 360, saturation * 100, lightness * 100);
        colors.push([rgb.r / 255, rgb.g / 255, rgb.b / 255, 1 ]); // Normalize to [0, 1]
    }
    
    return colors.sort(() => Math.random() * 3 - 1);
}
