export const UTIL_SHADER = /*wgsl*/ `
    ...
    fn fixPoint(vector: vec3<f32>) -> vec3<f32> {
        return vec3((vector.x) / renderState.ratio, vector.y, (vector.z + 1) / 2);
    }
`;
...
export const frag_vert_shader = /*wgsl*/ `
    ...
    @location(1) triangle_index: f32,
        ...
        var rotated: vec3f = vertex + gap * (piecePositions[i].xyz - og);
        ...
        if (pieceIsRotating[i] == 1) {
            ...
            switch u32(rotation_interpolation.y) {
                case 0: {
                    rotated = rotateY(radians(rotation_interpolation.x), rotated, gap * (center - og));                                                                   
                }
                case 1: {
                    rotated = rotateZ(radians(rotation_interpolation.x), rotated, gap * (center - og));
                }
                case 2: {
                    rotated = rotateX(radians(rotation_interpolation.x), rotated, gap * (center - og));
                }
                default: {}
            }
        }
        ...
        rotated = 
        rotateX(xRot, rotateY(yRot, rotated, vec3(0)), vec3(0));

        output.indices = vec2(i, u32(triangle_index));
        output.position = vec4(fixPoint(renderState.scale * rotated), 1.);
        return output;
    }
    ...
    fn getColor(frag: VertexOutput) -> vec4<f32> {
        ...
        for (var i: u32 = 0; i < arrayLength(&faceColors); i++) {
